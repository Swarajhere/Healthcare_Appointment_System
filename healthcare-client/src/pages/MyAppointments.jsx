...
// Optional: Use luxon for reliable IST handling
...
const MyAppointments = () => {
  ...
  // Current IST time using luxon (fallback to new Date if luxon not installed)
  const currentDateTime =
    DateTime.now().setZone("Asia/Kolkata").toJSDate() || new Date();
  const currentDate = format(currentDateTime, "yyyy-MM-dd");

  // Separate upcoming and completed appointments
  const upcomingAppointments = patientAppointments.filter((appt) => {
    const apptDateTime = new Date(`${appt.date}T${appt.time}:00+05:30`); // Explicit IST offset
    return (
